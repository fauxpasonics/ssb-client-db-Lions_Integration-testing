...
CREATE PROC [MERGEPROCESS_New].[MergeDetect_MSCRM] --'lions'
    @Client VARCHAR(100) 
AS
Declare @Date Date = (select cast(getdate() as date));
DECLARE @Account VARCHAR(100) = (SELECT CASE WHEN @client = 'lions' THEN 'CRM_Account' ELSE CONCAT(@client,' PC_SFDC Account' ) END);--updateme
DECLARE @Contact VARCHAR(100) = (SELECT CASE WHEN @client = 'lions' THEN 'CRM_Contact' ELSE CONCAT(@client,' PC_SFDC Account' ) END);--updateme
