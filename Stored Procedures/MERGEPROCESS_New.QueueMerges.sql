...
CREATE PROC [MERGEPROCESS_New].[QueueMerges] --lions
--Exec  [MERGEPROCESS_New].[QueueMerges] 'lions'
    @Client VARCHAR(100) 
AS
...
DECLARE @Contact VARCHAR(100) = (SELECT CASE WHEN @client = 'lions' THEN 'CRM_Contact' ELSE CONCAT(@client,' CRM Contact' ) END ); --updateme
DECLARE @Account VARCHAR(100) = (SELECT CASE WHEN @client = 'lions' THEN 'CRM_Account' ELSE CONCAT(@client,' CRM Account' ) END ); --updateme
--WITH only2Acct as
